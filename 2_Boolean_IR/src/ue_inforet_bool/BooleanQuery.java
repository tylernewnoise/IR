...
public class BooleanQuery {
	...
	private void parseTitleAndYear(int movieID, String mvLine, boolean isSeries) {
		...
		if (isSeries) {
			...
			st = new StringTokenizer(mvLine.substring(0, mvLine.length() - end), " .,:!?", false);
		}
...
/*		System.out.println("");
		...
		*//*if (queryString.contains(" AND ")) {
		...
		if (queryString.contains("\"")) {
			HashMap<Integer, Integer> phrases;
			// if phrase is for title
			if (queryString.indexOf("i") == 1) {
				token = "";
				// parse phrase first, start after 'title:"'
				for (int j = 7; j < queryString.length() - 1; j++) {
					// as delim...FUAAACK - just realized our index will not work for phrases
					if (queryString.charAt(j) == ' ') {
						if (!token.isEmpty()) {
							if (hashTitle.containsKey(token.toLowerCase())) {
								// get value from key(token)
								System.out.println(hashTitle.get(token.toLowerCase()));
							}
						}
						token = "";
						continue;
					}
					token += queryString.charAt(j);
				}
